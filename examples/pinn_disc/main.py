...
loss_history = []
def opt_cb(*args, **kwargs):
    loss_history.append(Jhat.functional.block_variable.checkpoint)

experiment.start_time = time.time()
weights, loss = train(Jhat, method="L-BFGS-B", callback=opt_cb, maxiter=args.maxiter, ftol=1e-20, tol=1e-20, gtol=1e-20)

with stop_annotating():
    net.set_weights(weights)
Jhat(weights)

weights, loss = train(Jhat, method="TNC", callback=opt_cb, maxiter=args.maxiter, ftol=1e-20, tol=1e-20, gtol=1e-20)
...
experiment.optimization_iteration_loss = loss_history
