...
model = TorchModel()
if args.eid is not None:
    model.u = experiment.NN[0]
    model.phi = experiment.NN[1]
    model.NN = experiment.NN[2]
model.double()

# Format: PDE, BC, IC, Data, L2 reg
if not hasattr(experiment, "torch_optim"):
...
optimizer = ScipyOptimizer(model.parameters(), method="TNC", callback=opt_cb, maxiter=args.maxiter,
                           maxfun=args.maxiter, disp=True, ftol=1e-20, gtol=1e-20, xtol=1e-20)
try:
    epochs = 1
    for epoch in range(epochs):
        print(f"Epoch = {epoch}")
...
experiment.NN = [model.u, model.phi, model.NN]
